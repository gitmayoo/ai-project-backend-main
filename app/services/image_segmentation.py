...
def segmenter(filename):
    ...
    segmented_image_path = os.path.join(app.config['SEGMENTED_IMGS'],to_be_segmented_part,filename)    # image_path = os.path.join(app.config['UPLOAD_FOLDER'],"53a013b7b03234d99cb20cf346f77b88.jpg")
...
def clothe_segmenter(filename,dress_type,dress_gender):
    filename = filename
    to_be_segmented_part = "dress"
    from app import create_app
    app = create_app()
    image_path = os.path.join(app.config['DRESS_IMGS'],f'{dress_gender}',f'{dress_type}',filename)
    file_path = os.path.join(os.path.dirname(__file__), '..', "models","selfie_segmenter.tflite")
